...
export async function POST(
  ...
  try {
    ...
    if (row.shipmentStatus !== "pending") {
      return NextResponse.json({ error: "Only pending items can be cancelled" }, { status: 400 });
    }
    ...
    const [updated] = await db
      ...
      .where(eq(orderItems.id, params.orderItemId))
      .returning({ id: orderItems.id, shipmentStatus: orderItems.shipmentStatus });
